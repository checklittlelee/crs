  ...
  <div>
    <div ref="wrapper" class="my-slider" :style="boxStyle">
      <ul
        ref="cont"
        class="cont"
        :style="isDefaultStyle ? defaultItemStyle : itemStyle"
      >
        <li
        ...
        >
          <div
            class="cont-img"
            :style="i === 0 ? '' : isDefaultStyle ? defaultImgStyle : imgStyle"
          >
            <van-image
              style="height: 80px"
              :radius="radius"
  ...
  methods: {
    ...
    setMyStyle() {
      ...
      if (!this.property.isDefaultMargin) {
        ...
        padding =
          `${this.property.padding[0]}px ${this.property.padding[1]}px` + ""
    ...
    verScroll() {
      let width =
        this.imageList.length * 100 +
        this.property.imageMargin * (this.imageList.length - 1) // 动态计算出滚动区域的大小，前面已经说过了，产生滚动的原因是滚动区域宽度大于父盒子宽度
